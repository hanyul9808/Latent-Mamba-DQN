...
from mamba_dqn_network import latentnomalMambaDQNNetwork
from replay_buffer import PrioritizedReplayBufferGPU


'''# --- 사용할 Mamba DQN 네트워크 클래스 선택 ---'''
SELECTED_MODEL_CLASS = latentnomalMambaDQNNetwork

MODEL_CLASS_NAME = SELECTED_MODEL_CLASS.__name__
AnyMambaDQNNetwork = Union[latentnomalMambaDQNNetwork]
...
if __name__ == "__main__":
    print(f"[Main] start (MambaDQN - Model: {MODEL_CLASS_NAME})")
    set_seed(SEED)
    device = get_device()

    print(f"[Main] '{ENV_NAME}'")
    env, local_input_dim = create_env(ENV_NAME)
    num_actions = env.action_space.n
    INPUT_DIM = local_input_dim 
    print(f"[Main]. Determined input_dim: {INPUT_DIM}, Actions: {num_actions}")
    ...
    print(f"[Main]. Input_dim: {INPUT_DIM}, Seq_len: {SEQUENCE_LENGTH}, Actions: {num_actions}, D_Model: {D_MODEL}")
    ...
    target_net.load_state_dict(main_net.state_dict())
    target_net.eval()
    print("[Main] Network initialization completed.")
    print("[Main] Initializing optimizer...")
    optimizer = optim.AdamW(main_net.parameters(), lr=LEARNING_RATE)
    print("[Main] Initializing optimizer completed.")

    print(f"[Main] Initializing replay buffer.. Input_dim: {INPUT_DIM}, Seq_len: {SEQUENCE_LENGTH}, Latent_dim: {D_MODEL}")
    replay_buffer = PrioritizedReplayBufferGPU(
    ...
    print(f"[Main] Initializing replay buffer completed. Buffer size: {BUFFER_SIZE}, Batch size: {BATCH_SIZE}")
    ...
    if resume_from and os.path.exists(resume_from):
        print(f"[Main] Resuming from checkpoint: {resume_from}")
    else:
        if resume_from:
            print(f"[Main] Checkpoint file not found: {resume_from}. Starting training from scratch.")
            resume_from = None
        else:
            print("[Main] Training will start from scratch (no checkpoint specified).")
          
    print("[Main] Preparing to call train_loop function...")
    ...
    try:
        ...
        print(f"[Main_ERROR] Unexpected error occurred in the main loop: {e}")
        import traceback
        traceback.print_exc()
    finally:
        print("[Main] Training loop terminated. Attempting to release resources...")
        if 'writer' in locals() and writer is not None:
            writer.close()
            print("[Main] TensorBoard writer has been closed.")
          
        if 'env' in locals() and env is not None:
            env.close()
            print("[Main] Environment has been closed.")
        if 'MODEL_CLASS_NAME' in locals() and 'current_max_grad_norm' in locals():
            print(f"[Main_Summary] Final network used: {MODEL_CLASS_NAME}, applied gradient clipping coefficient(max_norm): {current_max_grad_norm}")
        else:
            print("[Main_Summary] Failed to load network or gradient clipping information.")
